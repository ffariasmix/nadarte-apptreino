...
idProfessor=0 = TOTAL da unidade. Execucoes via resumo-execucoes-periodo com datas.
...
def coleta_unidade(uk, ulabel, key):
    ...
    # execucoes por dia da semana — PRECISA das datas

    execd = content(key, "/psec/treino-bi/resumo-execucoes-periodo/1?dataInicio=%d&dataFim=%d" % (di, df))
    ...
    epd = {d: num(execd, d) for d in

           ["domingo", "segunda", "terca", "quarta", "quinta", "sexta", "sabado"]} if isinstance(execd, dict) else {}

    exec_total = sum(v for v in epd.values() if isinstance(v, (int, float)))
    ...
    return {
        ...
        "treinosExecutados": exec_total,  # soma das execucoes por dia (fonte real do KPI1)
