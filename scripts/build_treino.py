...
def main():
    ...
    print("\n%-12s %8s %8s %9s %9s %9s %9s" % (

        "UNIDADE", "ALUNOS", "USO%APP", "EXEC", "VENCIDOS", "AVAL_OK", "AVAL_VEN"), file=sys.stderr)

    for u in data.get("unidades", []):

        print("%-12s %8s %8s %9s %9s %9s %9s" % (

            (u.get("nome") or "")[:12], sf(u.get("totalAlunos")), sf(u.get("percUtilizamApp")),

            sf(u.get("treinosExecutados")), sf(u.get("totalTreinosVencidos")),
